	...
	for (oix = 0; oix < ctx->nbufs; ++oix) {
		...
		if (ctx->maps[i]) {
			...
			pages[i].addr = ctx->buf->phys -
					...
					(pkt_size - rlen);
			pages[i].addr = (pages[i].addr & 0xFFFFFFFFULL) & PAGE_MASK;
			       ...
			       u32 kernel, uint32_t handle)
{
	...
	msg->handle = handle;
	msg->sc = ctx->sc;
	msg->addr = ctx->buf ? (ctx->buf->phys & 0xFFFFFFFFULL) : 0;
					      ...
					      char __user *argp)
{
	...
	pages[0].addr = fl->cctx->remote_heap->phys & 0xFFFFFFFFULL;
					...
					char __user *argp)
{
	...
	memlen = ALIGN(max(1024 * 1024 * 3, (int)init.filelen * 4), 1024 * 1024);
	...
	pages[0].addr = imem->phys & 0xFFFFFFFFULL;
...
static int fastrpc_req_mmap(struct fastrpc_user *fl, char __user *argp)
{
	...
	pages.addr = buf->phys & 0xFFFFFFFFULL;
	pages.size = buf->size;
...
static int fastrpc_req_mem_map(struct fastrpc_user *fl, char __user *argp)
{
	...
	pages.addr = map->phys & 0xFFFFFFFFULL;
	pages.size = map->len;
	...
	if (err) {
	    if (err == 0x80000414) {
	        dev_warn(dev, "DSP firmware does not support INIT_MEM_MAP, skipping\n");
	        err = 0;
	    } else {
